...
class Tokenizer():
    def __init__(self, ckpt_path: str):
        ...
        # Utils object for Vietnamese Word Segmentationpredict_funct (RDRVNDict)
        self.w_utils = WSUtils(ckpt['utils_args'])
        self.vectorizer_WS = WSCountVectorizer(
            utils=self.w_utils, 
            ratios=ckpt['ratios'], 
            feature_set=ckpt['feature_set']
        )
        self.vectorizer_WS.vocabulary_ = ckpt['vocabulary_']
        self.vectorizer_WS.fixed_vocabulary_ = ckpt['fixed_vocabulary_']
        self.vectorizer_WS.stop_words_ = ckpt['stop_words_']
        self.model_WS = ckpt['model_WS']
        self.seen_words = ckpt['seen_words']
        self.seen_sfx = ckpt['seen_sfx']
        del ckpt
    ...
    def segment(
        ...
        return self.w_utils.segment_words(
            self.model_WS,
            self.vectorizer_WS,
            texts,
            pre_tokenized,
